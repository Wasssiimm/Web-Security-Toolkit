...
function scrollToFaq() {
  ...
  if (!el) return
  const navbarHeight = 56 // matches h-14 in App.jsx
...
function FaqLink() {
  ...
  return (
    ...
    >
      ...
      <span className="-ml-3 group-hover:ml-0 transition-all">FAQ</span>
    </a>
  )
}
    ...
    <footer className="relative mt-24 border-t border-slate-200 dark:border-white/5 bg-white/30 dark:bg-slate-950/40 backdrop-blur-md">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-14 pb-6">
        ...
        <div className="mt-12 p-4 rounded-md border border-slate-200 dark:border-white/5 bg-slate-50/60 dark:bg-slate-900/40 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex gap-4 text-xs font-mono-cyber text-slate-500">
            <Link to="/terms"      className="hover:text-cyan-500 dark:hover:text-cyan-400 transition-colors">Terms</Link>
            <Link to="/privacy"    className="hover:text-cyan-500 dark:hover:text-cyan-400 transition-colors">Privacy</Link>
